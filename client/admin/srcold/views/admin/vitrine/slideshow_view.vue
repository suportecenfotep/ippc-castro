              ...
              <div class="col-12">
                <div class="card">
                  <div class="d-flex p-3">
                    ...
                    <button class = "btn btn-info btn-sm mx-1 fw-bold d-flex align-items-center p-1 rounded-3 border-0 text-uppercase">
                      <span class = "fas fa-file-pdf fs-4"></span> Ficha
                    </button>
                  </div>
                  <div class="card-body">
                      ...
                      <table id = "myDataTable" class="table table-striped">
                        ...
                        <tbody>
                            <tr v-for = "item in slideshows" :key = "item.id" @click="selectRow(item)" :class="{ 'selected': item === selectedRow }">
                               <td>{{item.title}}</td>
                               <td v-if="item.status">
                ...
                <div class="modal-dialog rounded-0">
                    <div class="modal-content rounded-0">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <p>
                                    Deseja excluir  <b>{{slideshow.title}}</b> definitivamente da base de dados?
                                </p>
                            </div>
                        </div>
                        <div class="modal-footer border-0 pt-0">
    ...
    export default{
        ...
        methods:{
            ...
            submitForm(){
              ...
              fd.append("title", this.slideshow.title)
              fd.append("descricao", this.slideshow.descricao)
              fd.append("status", this.slideshow.status)
              if(this.slideshow.id > 0){
                api
                .put(`/slideshow/${this.slideshow.id}`, fd)
                .then(res => {
                    if(res.data.message){
                        this.$swal.fire({
            ...
            deleteForm(){
                api
                .delete(`/slideshow/${this.slideshow.id}`)
                .then(res => {
                    if(res.data.message){
                        this.$swal.fire({
                          title:res.data.message,
